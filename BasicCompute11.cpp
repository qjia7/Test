...
int __cdecl main()
{
    ...
    if ( FAILED( CreateComputeShader( L"BasicCompute11.hlsl", "L3_SIMD_4x1_1x8", g_pDevice, &g_pCS ) ) )
        return 1;
    printf( "done\n" );
	...
	for (int it = 0; it < 20; it++)
	{
		...
		// Read back the result from GPU, verify its correctness against result computed by CPU
		{
			...
			float error = 0.0;
			for (int m = 0; m < OutputM; ++m)
			{
				for (int n = 0; n < OutputN; ++n)
				{
					float acc = 0.0;
					for (int k = 0; k < OutputK; k++)
					{
						acc += g_vBuf0[k*OutputM + m] * g_vBuf1[n*OutputK + k];
					}
					float extualAcc = p[n*OutputM + m];
					float er = fabs(extualAcc - acc);
					if ((extualAcc - acc) > 0.003 || (acc - extualAcc) > 0.003)
					{
						printf( "error at m%d,n%d\n", m,n);
					}
					error += er;
				}
			}
		...
		printf("Elapsed time in %I64d ms, %I64d us. C[%d, %d] = %f, %f\n",
			...
			std::chrono::duration_cast<std::chrono::microseconds>(diff).count(), m, n, result, acc);
	}
    printf( "Cleaning up...\n" );
